...
def main():
    ...
    parser.add_argument(
        "--cellmetadata",
        type=str,
        default="cell_metadata",
        help="relative path to the cell metadata folder",
    )
    parser.add_argument(
        "--study_column",
        type=str,
        default="datasetID",
        help="study column in tiledb, will be renamed to 'study'",
    )
    parser.add_argument(
        "--sample_column",
        type=str,
        default="sampleID",
        help="sample column in tiledb, will be renamed to 'sample'",
    )
    parser.add_argument(
        "--tissue_column",
        type=str,
        default="tissue",
        help="tissue column in tiledb, will be renamed to 'tissue'",
    )
    parser.add_argument(
        "--disease_column",
        type=str,
        default="disease",
        help="disease column in tiledb, will be renamed to 'disease'",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="An optional file for a safelist of cell type names in cell type prediction, one per line",
    )
    ...
    cell_metadata["author_label"] = (
        cell_metadata["cellTypeOntologyID"].map(id2name).astype(str)
    )
    ...
    embedding_tdb = tiledb.open(
        os.path.join(cellsearch_path, "cell_embedding"), "r", config=cfg
    )
    prediction_list = []
    ...
    for i in tqdm(range(0, cell_metadata.shape[0], buffer_size)):
        ...
        predictions, _, distances, _ = model.get_predictions_knn(
            embedding, disable_progress=True
        )
